...
output_cat = "dataset/predict/cat"
output_nocat = "dataset/predict/nocat"
os.makedirs(output_cat, exist_ok=True)
os.makedirs(output_nocat, exist_ok=True)

test_images = (test_data * 255.0).reshape(50, 64, 64, 3).astype(np.uint8)
...
for index in range(len(test_images)):
    ...
    if pred > 0.5:
        label = "cat"
        path = os.path.join(output_cat, f"pred_{index+1}.png")
    else:
        label = "nocat"
        path = os.path.join(output_nocat, f"pred_{index+1}.png")
    Image.fromarray(img).save(path)
    print(f"Imagen {index+1}: {' GATO' if label == 'cat' else ' NO GATO'} → {path}")

print("✅ Clasificación de imágenes completada.")